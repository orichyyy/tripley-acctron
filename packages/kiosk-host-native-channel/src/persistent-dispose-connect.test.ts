...
describe("persistent connect disposal", () => {
  it("keeps disposed terminal after an in-flight connect resolves", async () => {
    ...
    await expect(start).rejects.toThrow("host.session.disposed");
    expect(close).toHaveBeenCalledWith("late-socket");
