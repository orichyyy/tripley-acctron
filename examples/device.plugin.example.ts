export class IdCardReaderInputSourceAdapter implements InputSourceAdapter<IdCardReaderInputOptions, IdCardReaderInputResult> {
  ...
  async start(ctx: UserInputExecutionContext, source: UserInputSourceDefinition<IdCardReaderInputOptions>) {
    const operationId = ctx.idGenerator.create('id_card_reader');
    const reader = ctx.devices.get<IdCardReaderPort>('idCardReader');

    const result = reader.readIdentity(source.options, {
      operationId,
      ...
      safeSummary: { sourceKind: 'bank.idCardReader.identity', documentType: identity.documentType, hasTokenizedIdentity: true },
    }));
